#!/usr/bin/env node
/**
 * verify-docs-action — Verify that documentation linked from CLAUDE.md is up to date.
 ...
 *
 * Usage: verify-docs-action [options] [repo_dir]
 *
...
async function main(): Promise<void> {
  ...
  if (!existsSync(claudeMd)) {
    const msg = `ERROR: ${claudeMd} not found`;
    if (isGitHubAction()) {
      core.setFailed(msg);
    } else {
      console.error(msg);
  ...
  if (isGitHubAction()) {
    core.setOutput("files-checked", files.length.toString());
    core.setOutput("files-outdated", outdated.length.toString());
    core.setOutput("results-dir", resultsDir);
  }
    ...
    try {
      ...
      const prUrl = createPR(
        ...
        [
          ...
          "",
          "---",
          "_Created by `verify-docs-action`._",
        ].join("\n"),
      );
      ...
      if (isGitHubAction()) {
        core.warning(`Error fixing ${file}: ${message}`);
      }
    } finally {
      try {
        restoreMainBranch(repoDir);
  ...
  if (isGitHubAction()) {
    core.setOutput("prs-created", prsCreated.length.toString());
    core.setOutput("pr-urls", prsCreated.join("\n"));
  }
